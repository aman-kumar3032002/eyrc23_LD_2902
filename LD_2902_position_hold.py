	...
	#	8. Add error_sum
		...
		if(self.time_change >self.sample_time):
			if(self.last_time !=0):
				...
				self.error[0] = self.drone_position[0] - self.setpoint[0]   #for roll
				self.error[1] = self.drone_position[1] - self.setpoint[1]   #for pitch
				self.error[2] = self.drone_position[2] - self.setpoint[2]   #for throttl0
