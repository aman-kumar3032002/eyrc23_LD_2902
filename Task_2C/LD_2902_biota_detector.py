...
class swift():
    ...
    #-----------------------------Write the PID algorithm here--------------------------------------------------------------       
        ...
        if(self.time_change >self.sample_time):
            ...
            if(-0.2< self.error[0]<0.2 and -0.2< self.error[1]< 0.2 and -0.2< self.error[2]<0.2 and self.setpoint_index_changer < (len(self.setpoint)-1)):
                self.setpoint_index_changer = self.setpoint_index_changer+1                    #updating the index of the setpoint 
                
            if(-0.2< self.error[0]<0.2 and -0.2< self.error[1]< 0.2 and -0.2< self.error[2]<0.2 and len(self.contours)> 0 and self.isPublished == 0):
    ...
    called automatically when the gazebo is running
        ...
        for c,contour in enumerate(self.contours):
            ...
            contour_Y = (b+d/2)/107                                                             #contour_y , stores the y coordinate of the contour, (divided by 100 to get a sigle digit value)            
