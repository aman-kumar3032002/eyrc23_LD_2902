...
Filename: LD_2902_life_form_detector.py
Functions: disarm(), arm(), whycon_callback(), altitude_set_pid(), pitch_set_pid(), roll_set_pid(),pid(), hold_align_pid(), image_callback(), organism_type()
...
class swift():
    ...
    def __init__(self):
        ...
        self.drone_position = [0.0,0.0,0.0]	                             #drone_position, sets the drone position
        
        self.bridge = CvBridge()                                         #bridege, object of the CvBridge class

        # [x_setpoint, y_setpoint, z_setpoint]
        self.setpoint = [[0,0,0],[0,0,25],[8,0,25],[8,-8,25],[0,-8,25],[-8,-8,25],[-8,0,25],[-8,8,25],[0,8,25],[8,8,25],[11,11,32],[11,11,36],[11,11,37]
                        ]                                                
        ...
        #----------------------variables for the position_hold_pid----------------------------------------------------------------------------
        ...
        self.flag = 0                                                   #flage, stores the number of times the biolocation msg is published
        self.isPublished = False                                        #isPublished, stores whether the biolocation msg has been published or not   
        self.set_point_iterator = 0                                     #set_point_iterator, iterats the set_point list to change the location of the drone
    ...
    #-----------------------------Write the PID algorithm here--------------------------------------------------------------       
        ...
        if(self.time_change >self.sample_time):
            if(self.last_time !=0):
                ...
                self.error[0] = self.drone_position[0] - self.setpoint[self.set_point_iterator][0]   #for roll
                self.error[1] = self.drone_position[1] - self.setpoint[self.set_point_iterator][1]   #for pitch
                self.error[2] = self.drone_position[2] - self.setpoint[self.set_point_iterator][2]   #for throttl
            ...
            #changing the corrdinates of the drone if the condition is true------------------------------------------------
            if(-0.2< self.error[0]<0.2 and -0.2< self.error[1]< 0.2 and -0.2< self.error[2]<0.2 and self.set_point_iterator < (len(swift_drone.setpoint)-1)):
                self.set_point_iterator = self.set_point_iterator + 1                               #set_point_iterator, changes the index of the setpoint list everytime the condition gets true
                               
    def hold_align_pid(self):
        """
    Purpose:
    -------------------------------------------
    PID algorithm for the aligning the drone with the organism centroid
    ...
    Example Call:
    -------------------------------------------
    called automatically when any contour is detected
        """              
        self.now = int(round(time.time()*1000))                                                          #storing current time in senconds
        self.time_change = self.now - self.last_time									                 #difference between current time and last time
        ...
        if(self.time_change >self.sample_time):
            if(self.last_time !=0):
                ...
                #centroid allignment error, runs only if the msg has not been published yet and a centroid is detected
                if(self.isPublished == False):			
                    self.error[0] = self.frame_centeroids[0]-self.organism_centroids[0]                  #centroid error(pixel) for the roll axis
                    self.error[1] = self.frame_centeroids[1]-self.organism_centroids[1]                  #centroid error(pixel) for the pitch axis
                    
                    #no error check in the z coordinate as we have to align the camera frame centroid with the centroid of the organism
                    self.error[2] = self.drone_position[2] - self.setpoint[self.set_point_iterator][2]   #throttle 
                    
                #if the msg gets published, the error is checked only for the coordinates not for the organisms and camera               
                elif(self.isPublished == True ):
                    self.error[0] = self.drone_position[0] - self.setpoint[self.set_point_iterator][0]   #for roll
                    self.error[1] = self.drone_position[1] - self.setpoint[self.set_point_iterator][1]   #for pitch
                    self.error[2] = self.drone_position[2] - self.setpoint[self.set_point_iterator][2]   #for throttle
                    if(-0.2< self.error[0]<0.2 and -0.2< self.error[1]< 0.2 and -0.2< self.error[2]<0.2 and self.set_point_iterator < (len(swift_drone.setpoint)-1)):
                        self.set_point_iterator = self.set_point_iterator + 1                            #updating the index value by 1
            ...
            #publishing on the topic /astrobiolocation when the centroid of the organism gets alligned with the centroid of the drone camera frame
            if(-0.1< swift_drone.error[0]<0.1 and -0.1< swift_drone.error[1]< 0.1 and self.flag <1):
                ...
                #-----------------------------------------------------------------------------------------------------------------------
                self.flag = self.flag +1                                                    #updating the flag value 
                self.isPublished = True                                                     #updating the variable to True if msg is published
    #-----------------------------------------------------------------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():  
        ...
        #if no contour is detected, the drone will conver the given path using the pid fucntion------------------------------------------------------------------         
        if(swift_drone.set_point_iterator < (len(swift_drone.setpoint)-1) and len(swift_drone.contours)<1):
            swift_drone.pid()       
            
        #if a contour is detected the hold_align_pid will allign the drone to the centroid of the organism, publishes the msg and then follows the waypoint------
        elif(len(swift_drone.contours)>0):
            swift_drone.hold_align_pid()  
        
        #the loop will run when the drone is at the last set_point([11,11,37])-----------------------------------------------------------------------------------
        while(swift_drone.set_point_iterator == (len(swift_drone.setpoint)-1)):    
            #landing ---------------------------------------------------------------------------------------------------------------------------------------------    
            swift_drone.cmd.rcThrottle = 0                         
            swift_drone.cmd.rcRoll = 0
            swift_drone.cmd.rcPitch = 0                                    
            swift_drone.disarm()                                             #disarming the drone            
        r.sleep() 
