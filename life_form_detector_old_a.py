...
class swift():
    ...
    def __init__(self):
        ...
        self.contours = 0
        self.cv_image = np.empty([])
        self.centroid_cordinatess = []
        ...
        # self.setpoint = [[0,0,0],[0,0,30],
        #                 [2,0,30],[2,4,30],[-2,4,30],[-2,-4,30],[0,-4,30],
        #                 [5,-4,30],[5,0,30],[5,6,30],[0,6,30],[-5,6,30],[-5,0,30],[-5,-6,30],[0,-6,30],
        #                 [7,-6,30],[7,0,30],[7,8,30],[0,8,30],[-7,8,30],[-7,0,30],[-7,-8,30],[0,-8,30],
        #                 [7,-8,30],[7,8,30],[11,11,37.5],[11,11,37]]   
        
        self.setpoint = [[0,0,0],[0,0,30],
                        [2,4,30],[-2,4,30],[-2,-4,30],
                        [5,-4,30],[5,6,30],[-5,6,30],[-5,-8,30],
                        [6,-6,30],[7,8,30],[-7,8,30],[-7,-8,30],
                        [7,-8,30],[7,8,30],[11,11,37.5],[11,11,37]]  
        
        self.holdpositon = [0.0,0.0,0.0]	
        
        # self.setpoint = [[0,0,0],[0,0,30],
        #                 [2,0,30],[2,4,30],[-2,4,30],[-2,-4,30],[0,-4,30],
        #                 [5,-4,30],[5,0,30],[5,6,30],[0,6,30],[-5,6,30]]                  
        ...
        self.max_values = 1800                     #varibale to srtore the maximum valus of [roll,pitch,throttle]
        self.max_values_a = 2000
    ...
    #-----------------------------Write the PID algorithm here--------------------------------------------------------------
        ...
        if(self.time_change >self.sample_time):
            if(self.last_time !=0):
                ...
                self.error[2] = self.drone_position[2] - self.setpoint[self.i][2]   #for throttle
            ...
            if(-0.2< swift_drone.error[0]<0.2 and 
               -0.2< swift_drone.error[1]< 0.2 and 
               -0.2< swift_drone.error[2]<0.2 and 
               self.i < (len(swift_drone.setpoint)-1)):
                self.i = self.i+1 
    ...
    def pid_hold(self):
        ...
        if(self.time_change >self.sample_time):
            if(self.last_time !=0):
                #error of all the cordinates(for proportional)				
                self.error[0] = self.drone_position[0] - self.holdpositon[self.j][0]   #for roll
                self.error[1] = self.drone_position[1] - self.holdpositon[self.j][1]   #for pitch
                self.error[2] = self.drone_position[2] - self.holdpositon[self.j][2]   #for throttle
                self.alien.whycon_x = self.drone_position[0]
                self.alien.whycon_y = self.drone_position[1]
                self.alien.whycon_z = self.drone_position[2]
                ...
                self.cmd.rcRoll = 1500-int((self.Kp[0]*self.error[0])+(self.Kd[0]*self.error_diff[0]))		#roll 
                self.cmd.rcPitch = 1500+int((self.Kp[1]*self.error[1])+(self.Kd[1]*self.error_diff[1]))		#pitch
                self.cmd.rcThrottle = 1500+int((self.Kp[2]*self.error[2])+ 
                                               (self.Kd[2]*self.error_diff[2])+(self.error_sum[2]*self.Ki[2])) #throttle
            ...
            if(-0.2< swift_drone.error[0]<0.2 and 
               -0.2< swift_drone.error[1]< 0.2 and 
               -0.2< swift_drone.error[2]<0.2 and 
               self.j < (len(swift_drone.holdpositon)-1)):
                self.j = self.j+1 
    ...
    def image_callback(self, img_msg):
        ...
        y = int((self.cv_image.shape[0]-height)/2)
        ...
        cv.rectangle(self.cv_image,(x,y),(x+width,y+height),(0,255,0),2)
        contours = cv.findContours(threshold,cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)[-2] 
        
        # centroid_cordinatess = []    
        ...
        for c,contour in enumerate(contours):
            ...
            if (cX != 0 and cY != 0 ):
                self.centroid_cordinatess.append([cX,cY])
                
                
        for i,centroid in enumerate(self.centroid_cordinatess):
            
            distance = np.linalg.norm(np.array(centroid) - np.array(swift_drone.drone_position[:2]))
            # print(f"Contour {i+1} - Centroid: {centroid}, Distance from Drone: {distance}")
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        # swift_drone.pid()
        # elif(-0.2< swift_drone.error[0]<0.2 and 
        #      -0.2< swift_drone.error[1]< 0.2 and 
        #      -0.2< swift_drone.error[2]<0.2 and 
        #      swift_drone.i == (len(swift_drone.setpoint)-1)):
        #     swift_drone.disarm()
        #     break
        
        # if swift_drone.contours > 0:
            
        
        # if swift_drone.contours > 0 :
        #     swift_drone.setpoint = swift_drone.drone_position
        #     swift_drone.command_pub.publish(swift_drone.cmd)
            
        # else : swift_drone.pid()
        
        if swift_drone.contours > 0 :
            # swift_drone.disarm()
            positiontohold = swift_drone.setpoint[swift_drone.i]
            # postiontoalign = [swift_drone.setpoint[swift_drone.i][0]+swift_drone.centroid_cordinatess[0],
            #                   swift_drone.setpoint[swift_drone.i][1]+swift_drone.centroid_cordinatess[1]
            #                     for i in range(2)]
            postiontoalignX = [swift_drone.setpoint[swift_drone.i][0] + swift_drone.centroid_cordinatess[0][i] for i in range(1)]
            postiontoalignY = [swift_drone.setpoint[swift_drone.i][1] + swift_drone.centroid_cordinatess[1][i] for i in range(1)]
            
            positiontoalign = [postiontoalignX[0],postiontoalignY[0],30]

            swift_drone.holdpositon = [positiontohold,positiontoalign]
            # swift_drone.i = 1
            # swift_drone.command_pub.publish(swift_drone.cmd)
            # swift_drone.arm()
            # swift_drone.hold_drone(swift_drone.drone_position)
            swift_drone.pid_hold()
            print("positiontohold",swift_drone.holdpositon)
            print("positiontoalign",positiontoalign)
        ...
        # print("contour",len(swift_drone.contours))
        
        print("contour",swift_drone.contours)
        print("alien",swift_drone.alien.organism_type)
        print("position",swift_drone.drone_position)
        print("setpoint",swift_drone.setpoint[swift_drone.i])
        print("holdposition",swift_drone.holdpositon[swift_drone.j])
